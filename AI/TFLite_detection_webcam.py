...
while True:
    ...
    #2021-02-23 camera frame num
    camfrmnum = camfrmnum + 1
    ...
    if camfrmnum == 1:
        ...
        filename = dtwithoutsecondnow.strftime('%Y-%m-%d_%H:%M:%S')
        output = filename + '.avi'
        video = cv2.VideoWriter(output,videostream.fourcc,30.0,(width,height))
    ...
    strLight1.addCarNum(carnum + trucknum)
    strLight1.setTotalNum()
    
    #2021-02-09 가로등 밝기 변화 체크
    
    is_Led_On = strLight1.getLedStatus()
    if is_Led_On == False:
        strLight1.setLedOn()
        strLight2.setLedOn()
    #strLight1.setLedBrt(strLight1.getTotalNum()*5)
    strLight2.setLedBrt(strLight2.getPeopleAvgNum()*5)
    
    if strLight1.getFrmNum() >= 10:
        print(strLight1.getPeopleAvgNum())
        print(strLight1.getCarAvgNum())
        #2021-03-31 HSJ w_brt 계산 및 밝기 조절
        strLight1.getWeather()
        strLight1.calByTra()
        strLight1.calByWea()
